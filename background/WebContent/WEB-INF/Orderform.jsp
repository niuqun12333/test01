...
$().ready(function() {
	$(".btn_search").click(function name() {
		var orderNum=$(".text_add").val();
		var time=$("#start").val();
		location.href="showOrderform.do?orderNum="+orderNum+"&time="+time;
	})
						...
						<tbody>
							<c:forEach items="${orderList}" var="order">
								<tr>
									...
									<a href="#"><img src="pic/${detail.product.pictures[0].path}" title="${detail.product.name}" /></a> 
									<c:if test="${!vs.last}">
									...
									<td class="td-status"><c:if
											test="${order.orCondition=='已发货'}">
											<span class="label label-success radius">已发货</span></td>
									</c:if>
									<c:if test="${order.orCondition!='已发货'}">
										<span class="label label-success radius">代发货</span>
										</td>
									</c:if>
									<td><c:if test="${order.orCondition!='已发货'}">
											<a onClick="Delivery_stop(this,'10034')" href="javascript:;"
												title="发货" class="btn btn-xs btn-success conditionBtn"
												data-id="${order.id}"><i class="fa fa-cubes bigger-120"></i></a>
										</c:if> <a title="订单详细" href="showOrderDetail.do?id=${order.id}"
										...
										class="btn btn-xs btn-warning deleteBtn"><i
