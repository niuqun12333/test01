...
<body>
				...
				<form action="addProduct.do" method="post"
					...
					<div class="clearfix cl">
						...
						<div class="formControls col-10">
							<input type="text" class="input-text" value="${pro.name}"
					...
					<div class=" clearfix cl">
						...
						<div class="formControls col-10">
							<input type="text" class="input-text" value="${pro.introduce}"
								placeholder="" id="introduce" name="introduce">
						</div>
					</div>
					<div class=" clearfix cl">
						...
						<div class="Add_p_s">
							...
							<div class="formControls col-2">
								<input type="text" class="input-text" value="${pro.id}"
						...
						<div class="Add_p_s">
							...
							<div class="formControls col-2">
								<input type="text" class="input-text" value="${pro.price}"
									placeholder="" id="price" name="price">元
							</div>
						</div>
						<div class="Add_p_s">
							<label class="form-label col-2">现有库存：</label>
							<div class="formControls col-2">
								<input type="text" class="input-text" value="${pro.store}"
					...
					<div class="clearfix cl">
						...
						<div class="form-label col-1">
							<input type="button" value="上传" id="upload"
								class="uploadBtn state-ready" />
						</div>
					</div>
					<div class="clearfix cl" id="pictures" data-id="${pro.id}">
						<c:forEach items="${pro.pictures}" var="pic">
							<img src="pic/${pic.path}" data-imgid="${pic.id}"
								data-name="${pic.path}" class="oldPicture" />
						</c:forEach>
					</div>
					<div class="clearfix cl">
	...
	<script type="text/javascript"
		src="Widget/ueditor/1.4.3/ueditor.all.min.js"> </script>
	<script type="text/javascript"
...
(function( $ ){
    ...
    var picNames="";
	var picIds="";
	/*function addPictures(picNames){
		$.ajax({
			url : "addPictures.do",
			type : "post",  
	        data:JSON.stringify(picNames), 
	...
	$("#upload").click(function() {
		...
		for (var i = 0; i < $("[name=files]")[0].files.length; i++) {
			formData.append("file", $("[name=files]")[0].files[i]);
		}
		$.ajax({
			url : "upload.do",
			type : "post",
			...
			success : function(data) {
				picNames+=data+",";
				var str = "<img src='pic/"+data+"' data-name='"+data+"'/>";  
				$("#pictures").append(str);
			}
		})
	})
	$(".btn-primary").click(function() {
		...
		oldPicNames=oldPicNames.substring(0,oldPicNames.length-1);
	...
	$(document).on("click","#pictures img",function(){
		if($(this).hasClass("oldPicture")){
			var picName=$(this).data("name");
			var picId=$(this).data("imgid");
			$(this).remove();
			$.ajax({
			...
			var picName=$(this).data("name");
			var str=picName+",";//声明变量
			picNames=picNames.replace(str,""); //将a字符替换为空字符
			$(this).remove();
			picDelete(picName);
		}
