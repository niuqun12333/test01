...
$().ready(function () {
	$(".btn_search").click(function() {
		var proName=$(".text_add").val();
		location.href="showProduct.do?name="+proName;
	})
	...
	<div class=" page-content clearfix">
		<div id="products_style">
			...
			<div class="border clearfix">
				<span class="l_f"> <a href="showPictureAdd.do" title="添加商品"
					class="btn btn-warning Order_form"><i class="icon-plus"></i>添加商品</a>
					<a href="javascript:ovid()" class="btn btn-danger deleteBath"><i
							...
							<c:forEach items="${proList}" var="pro">
								<tr>
									<td width="25px"><label><input type="checkbox"
											class="ace check" data-id="${pro.id}"><span
												class="lbl"></span></label></td>
									<td width="80px">${pro.id}</td>
									<td width="250px"><u style="cursor: pointer"
										class="text-primary" onclick="">${pro.name}</u></td>
									...
									<td class="td-manage"><a
										...
										href="showProductUpdate.do?id=${pro.id}"
										class="btn btn-xs btn-info Order_form"><i
											class="icon-edit bigger-120"></i></a> <a title="删除"
										href="javascript:;" onclick="member_del(this,'1')"
										class="btn btn-xs btn-warning deleteBtn" data-id="${pro.id}"><i
...
var zNodes =[
	...
	{ id:1${vs.index}, pId:1, name:"${cla.name}",file:"showProduct.do?c_id=${cla.id}",open:true},
	<c:forEach items="${cla.mcList}" var="mc" varStatus="mcvs">
	{ id:1${vs.index}${mcvs.index}, pId:1${vs.index}, name:"${mc.name}",file:"showProduct.do?c_id=${cla.id}"}, 
	</c:forEach>
	</c:forEach>
];
