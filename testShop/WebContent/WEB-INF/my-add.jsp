...
<script type="text/javascript">
	$().ready(function() {
		allPrice();
		var selectAddressId=-1;
		function allNumber(){
			var aNumber=0;
			$(".number").each(function(index, element) {
				aNumber+=parseFloat($(this).data("num"));
			})
		}
		function allPrice(){
			var aPrice=0;
			$(".subtotal").each(function(index, element) {
				aPrice+=parseFloat($(this).data("price"));
				})
				$("#totalPrice").html(aPrice+"元");
			    $("#PayPrice").html(aPrice);		
		}
		...
		$("#Provinces").bind("change",function(){
	        	   ...
	        	   $.ajax({
	            	   url:"searchCity.do",
	    			    ...
	    			    success:function(data){
	    			    	$("#Citys").html("");
	    			    	var str = "<option value=''>城市/地区/自治州</option>"; 
	    			    	$("#Citys").append(str);
	    			    	var str2 = "";
	    			    	$.each(data,function(i,result){
	    			    		str2="<option value='"+data[i].cityID+"'>"+data[i].city+"</option>";
	    			    		$("#Citys").append(str2);
	    			        }) 
	    			     }
	    			 })
	        	   ...
	        	   $.ajax({
	            	   url:"searchArea.do",
	    			    ...
	    			    success:function(data){
	    			    	...
	    			    	var str2 = "";
	    			    	$.each(data,function(i,result){
	    			    		str2="<option value='"+data[i].areaID+"'>"+data[i].area+"</option>";
	    			    		$("#Countys").append(str2);
	    			        }) 
	    			     }
	    			 })
	        ...
	        $("#J_editAddrOk").click(function() {
	        	var province = $("#Provinces").find("option:selected").html();
	        	var city = $("#Citys").find("option:selected").html();
	        	var area=$("#Countys").find("option:selected").html();
		    ...
		    var length = $("#checkoutAddrList .selected").length; 
		    if(length>0){
		    	selectAddressId = $(this).data("id");
				} else{
					selectAddressId = -1;
				}
		    })
		    
		    $("#lijixiadan").click(function() {
		    	...
		    	var loc_id=selectAddressId;
		    	var total=aPrice;
		    	var allNum=aNumber;
		    	var cdIds="${proIds}";
		    	location.href="addOrder.do?loc_id="+loc_id+"&total="+total+"&allNum="+allNum+"&cdIds="+cdIds;
		    	}else{
		    		alert("请选中收货地址!");
		    	}
			})
...
<body>
	...
	<div class="pc-header-logo clearfix">
		...
		<div class="fr pc-head-car">
			<i class="icon-car"></i> <a href="searchCar.do">我的购物车</a>
		</div>
	</div>
	<!--  顶部    start-->
	<div class="yHeader">
							...
							<div class="xm-box">
								...
								<div class="box-bd">
									<div class="clearfix xm-address-list" id="checkoutAddrList">
										...
										<dl class="item location" data-id="${location.id}">
											<dt>
												<strong class="itemConsignee">${location.name}</strong> <span
													class="itemTag tag">${location.tag}</span>
											</dt>
											<dd>
												<p class="tel itemTel">${location.phone}</p>
												<p class="itemRegion">${location.province} ${location.city} ${location.area}</p>
												<p class="itemStreet">${location.street}</p>
												<span class="edit-btn J_editAddr" data-id="${location.id}" >删除</span>
											</dd>
										...
										<div class="bd">
											...
											<div class="item">
												<label>地址<span>*</span></label> <select
													...
													<c:forEach items="${proList}" var="prov">
						...
						<div class="checkout-box-ft">
											...
											<c:forEach items="${list}" var="cd">
												<dd class="item clearfix">
													<div class="item-row">
														<div class="col col-1">
															<div class="g-pic">
																<img src="pic/${cd.product.pictures[0].path}" width="40"
																	height="40" />
															</div>
															<div class="g-info">
																<a href="#" target="_blank"> ${cd.product.name} </a>
															</div>
														</div>
														<div class="col col-2">${cd.product.price}元</div>
														<div class="col col-3 number" data-num="${cd.number}">${cd.number}</div>
														<div class="col col-4 subtotal" data-price="${cd.subtotal}">${cd.subtotal}元</div>
													</div>
												</dd>
											</c:forEach>
										</dl>
										<div class="checkout-count clearfix">
											...
											<div class="checkout-price">
												<ul>
													...
													<li>订单总额：<span id="totalPrice">${allPrice}元</span>
													</li>
												...
												<p class="checkout-total">
													应付总额：<span><strong id="PayPrice">${allPrice}</strong>元</span>
												</p>
											</div>
											<!--  -->
										</div>
							...
							<div class="checkout-confirm">
								...
								<a href="searchCar.do"
									...
									<a class="btn btn-primary" id="lijixiadan">立即下单</a>
