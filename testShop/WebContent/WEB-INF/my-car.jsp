	...
	$().ready(function() {
		...
		function allNum() {
			...
			$(".subtotal").each(function(index, element) {
				if($(this).parent().find(".check").prop("checked")==true){
			...
			$(".pc-shop-shu").html(allNum);
			$(".total").html("¥"+allPrice);
		}
		$(".buy").click(function () {
			var ids=str;
			var allNum=orderNum;
			var allPrice=orderPrice;
			if(ids!=""){
			location.href="showOrder.do?ids="+ids+"&allNum="+allNum+"&allPrice="+allPrice;
			}else{
				alert("请选中一个商品!");
			}
		...
		$(".reduce").click(function() {
			...
			var price = parseFloat($(this).parent().prev().find(".red").html());
		...
		$("#deleteSelected").click(function() {
			var cdIds =str;
			location.href="deleteCarDetails.do?cdIds="+cdIds;
		})
				...
				<thead>
					<tr class="tab-0">
						<th class="tab-1"><input type="checkbox" name="s_all"
							class="s_all tr_checkmr selectall" id="s_all_h"><label for="">
								全选</label></th>
					...
					<c:forEach items="${cds}" var="cd" varStatus="status">
						<tr>
							<th><input type="checkbox" class="check"
								style="margin-left: 10px; float: left"></th>
							<th class="tab-th-1"><a href="#"><img
									src="pic/${cd.product.pictures[0].path}" width="100%" alt=""></a>
								<a href="#" class="tab-title">${cd.product.name} </a></th>
							<th>
								<p>颜色：黑色</p>
								<p>规格：落地款</p>
							</th>
							<th>
								<p>${cd.product.price}</p>
								<p class="red">${cd.product.price}</p>
							</th>
							<th class="tab-th-2"><span class="reduce">-</span> <input
								type="text" value="${cd.number}" maxlength="3" placeholder=""
								class="shul num" data-id="${cd.id}" data-pid="${cd.product.id}">
								<span class="add">+</span></th>
							<th class="red subtotal">${cd.subtotal}</th>
							<th><a href="deleteCar.do?id=${cd.id}">删除</a></th>
	...
	<div class="center">
		<div class="clearfix pc-shop-go">
			<div class="fl pc-shop-fl">
				<input type="checkbox" placeholder="" class="selectall"> <label for="">全选</label>
				<a id="deleteSelected">删除选中</a> <a href="#">清楚失效商品</a>
			</div>
			<div class="fr pc-shop-fr">
				<p>
					共有 <em class="red pc-shop-shu">${car.allNum}</em> 款商品，总计（不含运费）
				</p>
				<span class="total" data-id="${car.id}">¥${car.total}</span> <a class="buy">去付款</a>
			</div>
		</div>
	</div>
	</section>
