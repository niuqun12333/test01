...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$(".confirm").click(function() {
			var ye = $(".pc-search-j").val();
			$(this).attr("href","classSearch.do?ye="+ye+"&name=${c.name}&fprice=${fprice}&sprice=${sprice}")
		})

		if(${p.ye} <= 1){
			$(".pre").addClass("pc-search-f pc-search-d");
		    $(".pre").attr("onclick","return false");
		}
		
		if(${p.ye} >= ${p.maxYe}){  
			$(".next").addClass("pc-search-f pc-search-d");
				...
				<div class="fl">
					<div id="firstpane" class="menu_list">
						...
						<div style="display: block" class="menu_body">
							<a href="#">冲调饮品</a> <a href="#">糖果/巧克力</a>
						</div>
						<h3 class="menu_head">营养健康</h3>
						<div style="display: none" class="menu_body">
							<a href="#">营养健康</a> <a href="#">营养健康</a> <a href="#">营养健康</a>
						</div>

						<h3 class="menu_head">粮油调味</h3>
						<div style="display: none" class="menu_body">
							<a href="#">粮油调味</a> <a href="#">粮油调味</a> <a href="#">粮油调味</a> <a
								href="#">粮油调味</a>
						</div>
					</div>
				</div>
					...
					<div class="pc-term">
						...
						<div class="pc-search clearfix">
							<div class="fl pc-search-in">
								<form action="classSearch.do">
									<input class="pc-search-w" type="text" name="name"> <input
										class="pc-search-s" placeholder="￥" type="text" name="fprice" />
									<input class="pc-search-s" placeholder="￥" type="text"
										name="sprice" /> <input class="pc-search-a" type="submit"
										value="搜索" />
								</form>
							</div>
							<div class="fr pc-with">
								相关搜索： <a href="#">黑糖</a><em>|</em><a href="#">姜茶</a><em>|</em><a
									...
									href="#">黑糖姜母</a><em>|</em><a href="#">茶黑糖饼</a><em>|</em><a
									href="#">干黑糖</a><em>|</em><a href="#">沙琪玛</a>
							</div>
						</div>
					</div>
					<div class="pc-term">
						<div class="clearfix pc-search-p">
							<div class="fl pc-search-e">
								<a href="#" class="cur">销量</a><a href="#">价格</a><a href="#">评价</a><a
									href="#">上架时间</a>
							</div>
						...
						<div class="pc-search-i">
							<div class="fr">
								...
								<a
									href="classSearch.do?ye=${p.ye-1}&name=${c.name}&fprice=${fprice}&sprice=${sprice}"
									class=" pre">上一页</a> <a class="next"
									href="classSearch.do?ye=${p.ye+1}&name=${c.name}&fprice=${fprice}&sprice=${sprice}">下一页</a>
							</div>
						</div>
					</div>
					<div class="time-border-list pc-search-list clearfix">
						<ul class="clearfix">
							<c:forEach items="${list}" var="pro">
								<li>
								<c:if test="${pro.pictures[0].path ==null}">
								<a href="pageSearch.do?id=${pro.id}"> <img src="images/shangpinxiangqing/X-1.png" width=100%></a>
								</c:if>
								<c:if test="${pro.pictures[0].path !=null}">
								<a href="pageSearch.do?id=${pro.id}"> <img src="pic/${pro.pictures[0].path}" width=100%></a>
								</c:if>
									<p class="head-name">
										<a href="#">${pro.name}</a>
									</p>
									...
									<p class="head-futi clearfix">
										<span class="fl">好评度：90% </span> <span class="fr">${pro.sellNum}人购买</span>
									</p>
									<p class="clearfix">
										<span class="label-default fl">抢购</span> <a href="#"
						...
						<div class="clearfix">
							<div class="fr pc-search-g">
								<a class="fl  pre"
									href="classSearch.do?ye=${p.ye-1}&name=${c.name}&fprice=${fprice}&sprice=${sprice}">上一页</a>
								<c:forEach begin="${p.beginYe}" end="${p.endYe}"
									varStatus="status">
									<a
										href="classSearch.do?ye=${status.index}&name=${c.name}&fprice=${fprice}&sprice=${sprice}"
										<c:if test="${p.ye==status.index}">class="current"</c:if>>${status.index}</a>
								</c:forEach>
								<span class="pc-search-di">…</span> <a title="使用方向键右键也可翻到下一页哦！"
									class="next"
									href="classSearch.do?ye=${p.ye+1}&name=${c.name}&fprice=${fprice}&sprice=${sprice}"
									onclick="SEARCH.page(3, true)">下一页</a> <span
									class="pc-search-y"> <em> 共${p.maxYe}页 到第</em> <input
									class="pc-search-j" placeholder="1" type="text"> <em>页</em>
					...
					<div class="pc-search-re clearfix">
						<dl>
							...
							<dd>
								...
								<input value="搜&nbsp;索" id="btn-re-search" class="button"
									type="button">
							</dd>
						</dl>
					</div>
