	...
	<div id="main">
		<div id="select">
			<form action="score" method="post">
				<div class="form-group">
					<div>
						<input type="text" class="col-sm-4 form-control" name="empName"
							placeholder="请输入姓名"
							<c:if test="${sc.emp.name!=''}">value="${sc.emp.name}"</c:if>>
					</div>
					<div>
						<select name="dep" class="col-sm-4 form-control">
							...
							<c:forEach items="${dep}" var="deps">
								<option value="${deps.id}"
									<c:if test="${sc.emp.dep.id==deps.id}">selected</c:if>>${deps.name}</option>
							</c:forEach>
						</select>
					</div>
					<div>
						<select name="pro" class="col-sm-4 form-control">
							...
							<c:forEach items="${pro}" var="pros">
								<option value="${pros.id}"
									<c:if test="${sc.pro.id==pros.id}">selected</c:if>>${pros.name}</option>
							</c:forEach>
						</select>
					</div>
					<div>
						<select name="grade" class="col-sm-4 form-control">
							...
							<option value="不及格"
								<c:if test="${sc.grade=='不及格'}">selected</c:if>>不及格</option>
						</select>
					</div>
			...
			<tbody>
				<c:forEach items="${scList}" var="scs">
					<tr class="emp" data-id="${scs.id}">
						...
						<input type="hidden" name="depCount"
							value="${scs.emp.dep.emp_count}">
						...
						<td class="valueTd">
							<p class="value">${scs.value}</p> <input type="text"
							value="${scs.value}" name="updateValue" style="display: none;" />
						</td>
						<td class="gradeTd">
		...
		<div id="page">
			<div id="pages">
				<ul class="pagination">
					<li id="begin"><a
						href="score?ye=1&empName=${sc.emp.name}&dep=${sc.emp.dep.id!=-1?sc.emp.dep.id:''}&pro=${sc.pro.id!=-1?sc.pro.id:''}">首页</a></li>
					<li id="pre"><a
						href="score?ye=${p.ye-1}&empName=${sc.emp.name}&dep=${sc.emp.dep.id!=-1?sc.emp.dep.id:''}&pro=${sc.pro.id!=-1?sc.pro.id:''}">上一页</a></li>
					...
					<li id="next"><a
						href="score?ye=${p.ye+1}&empName=${sc.emp.name}&dep=${sc.emp.dep.id!=-1?sc.emp.dep.id:''}&pro=${sc.pro.id!=-1?sc.pro.id:''}">下一页</a></li>
					<li id="end"><a
						href="score?ye=${p.maxYe}&empName=${sc.emp.name}&dep=${sc.emp.dep.id!=-1?sc.emp.dep.id:''}&pro=${sc.pro.id!=-1?sc.pro.id:''}">末页</a></li>
				</ul>
			</div>
			<div id="message">
				<p>共${p.maxYe}页</p>
